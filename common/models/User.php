...
class User extends ActiveRecord implements IdentityInterface
{
    ...
    public static function  generateAcessToken($user){
        ...
        $userToken = Token::find()->where(["user_id" => $user->id])->one();

        if (!$userToken) {
            $userToken =  new Token();
        }

        $userToken->token = md5(microtime());
        $userToken->expires_at = strtotime('+  15 minutes ', time());
    ...
    public function findToken($token){
        $userToken = Token::find()->where(["token" => $token])->one();
        if($userToken){
            return true;
        }
    ...
    public function findUserByToken($token){
         ...
         return User::find()->where(["id" => $token->user_id])->one();
    ...
    public static function isActive($token){
        ...
        $user =  Token::find()->where(["token" => $token])->one();
